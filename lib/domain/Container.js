...
class Container {
    ...
    deleteByQuery(conditionQuery, getIdFn = GetIdDefault, getPartitionKeyFn = GetPartitionKeyDefault) {
        assert.ok(conditionQuery, "Query is not defined");
        assert.ok(getIdFn, "Get Id is not defined");

        return printProcessor(async () => {
            const loadedItems = await this.loadByQuery(`select * from c where ` + conditionQuery);
    ...
    updateByQuery(conditionQuery, replaceObjOrFn, getPartitionKeyFn = GetPartitionKeyDefault) {
        assert.ok(conditionQuery, "Query is not defined");
        ...
        return printProcessor(async () => {
            ...
            {
                noPrintResult: true,
                loading: `=>[${this._collectionName}] Executing Update By Query: ${conditionQuery} - ${JSON.stringify(replaceObjOrFn, null, 4)}`
            });
    }

    updateByList(registers, getPartitionKeyFn = GetPartitionKeyDefault) {
        ...
        return printProcessor(async () => {
            ...
            {
                noPrintResult: true,
                loading: `=>[${this._collectionName}] Executing Update By List[${registers.length}]`
            });
    }
